...
begin
  ...
  perform private.log_event(pl.game_id, 'eliminated',
    pl.name || case p_cause when 'storm' then ' was eliminated by the storm'
                            when 'gamemaker' then ' was eliminated'
                            else ' was eliminated' end,
    'all', pl.id, pl.team_id, jsonb_build_object('cause', p_cause));
  perform private.notify_player(pl.id, 'You''ve been eliminated',
    case p_cause when 'storm' then 'The storm got you.' else 'A gamemaker marked you as eliminated.' end, 'death');
  if p_cause = 'storm' then
    perform private.notify_gms(pl.game_id, 'Storm elimination', pl.name || ' died in the storm.', 'death');
  end if;
...
create or replace function public.gm_set_player_status(p_player uuid, p_status text) returns void
language plpgsql security definer set search_path = '' as $$
declare
...
begin
  ...
  if p_status = 'dead' then
    perform private.eliminate(p_player, 'gamemaker');
...
-- p_storm is the full circle plan generated by the gamemaker's client (random circles are rolled there).
